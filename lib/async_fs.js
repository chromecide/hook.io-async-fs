...
var async                 = require('async')
  ...
  ;
//require('hook.io');
var FileHook  = require('./filehook').FileHook;

//exports.FileHook  = FileHook;

var async_fs = exports.async_fs = function(options){
	
	Hook.call(this, options);
	var self = this;
	
	self.on('*::'+self.name+'::async_fs::watch_file', self.watchFile);
	self.on('*::'+self.name+'::async_fs::unwatch_file', self.unwatchFile);
	self.on('*::'+self.name+'::async_fs::watch_tree', self.watchTree);
	
	self.on('hook::ready', function(){
		if(self.hookio_async_fs){
			if(self.hookio_async_fs.watch && self.hookio_async_fs.watch.length>0){
				for(var fIdx in self.hookio_async_fs.watch){
					var fPath = self.hookio_async_fs.watch[fIdx];
					self.watchFile({path: fPath});
				}
			}
		}
		
		if(self.watch && self.watch.length>0){
			if(self.watch && self.watch.length>0){
				for(var fIdx in self.watch){
					var fPath = self.watch[fIdx];
					self.watchFile({path: fPath});
				}
			}
		}
...
var cache = [];
	...
	util.inherits(async_fs, Hook);

function watchFile_asyncFs(options) {
  var self = this;
  var dir_path = options.path;
  if(options.path && cache[options.path]){
  	return options.path;
  }
  var event_emitter;
  //if it's a directory, call watch tree instead
  isDirTest_asyncFs(options.path, function(err, filename){
  	if(filename!='' && filename!==undefined){//directory
  		event_emitter = self.watchTree(options);
  		event_emitter.start();
  	}else{ //file
  		event_emitter = new FileHook(options);	
  		event_emitter.start();	
  	}
  });
  
  cache[options.path] = event_emitter;
  return event_emitter;
}

async_fs.prototype.watchFile = watchFile_asyncFs;

async_fs.prototype.unwatchFile = function unwatchFile_asyncFs(options) {
  var dir_path = options.path;
 ...
 *  
 */
async_fs.prototype.watchTree = function watchTree_asyncFs(options) {
	var self = this;
  var dir_path = options.path;
  ...
  fs.readdir(dir_path, function(err, files) {
    ...
    dir_event_emitter.on('async-fs::file::add', function(data){
    	//add a hook for the new file
    	self.watchFile(data);
    });
 ...
 *
 */
function isDirTest_asyncFs(file_path, callback) {
 ...
 */
function forEachFile(path, iterator, callback) {
 ...
 * @param callback A callback of the form `function(err)`.
 */
function forEachFileSeries(path, iterator, callback) {
